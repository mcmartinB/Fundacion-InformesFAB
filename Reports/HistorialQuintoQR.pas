...
type
  TQRHistorialQuinto = class(TQuickRep)
...
var
  QRHistorialQuinto: TQRHistorialQuinto;
...
procedure TQRHistorialQuinto.qrlbl24Print(sender: TObject;
  var Value: String);
begin
  Value:= '';
...
procedure TQRHistorialQuinto.LoadData( const AFechaPromocion, AFechaHistorial: string );
begin
  sFechaPromocion:= AFechaPromocion;
end;

procedure TQRHistorialQuinto.qrlblFechaPromESOPrint(sender: TObject;
  var Value: String);
begin
  //
...
procedure TQRHistorialQuinto.PonNOta(sender: TObject; var Value: String);
begin
...
procedure TQRHistorialQuinto.ponNotaValenciano(sender: TObject;
  var Value: String);
begin
  if DataSet.FieldByname(TQRDBText( sender ).DataField).AsFloat <> -1 then
