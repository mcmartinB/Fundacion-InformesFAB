...
type
  TQRHistorialSegundo = class(TQuickRep)
...
var
  QRHistorialSegundo: TQRHistorialSegundo;
...
procedure TQRHistorialSegundo.qrlbl24Print(sender: TObject;
  var Value: String);
begin
  Value:= '';
...
procedure TQRHistorialSegundo.LoadData( const AFechaPromocion, AFechaHistorial: string );
begin
  sFechaPromocion:= AFechaPromocion;
end;


procedure TQRHistorialSegundo.qrlbl54Print(sender: TObject;
  var Value: String);
begin
  //
...
procedure TQRHistorialSegundo.PonNota(sender: TObject; var Value: String);
begin
...
procedure TQRHistorialSegundo.PonNotaValenciano(sender: TObject;
  var Value: String);
begin
  if DataSet.FieldByname(TQRDBText( sender ).DataField).AsFloat <> -1 then
